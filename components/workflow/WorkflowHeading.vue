...
<template>
  <div>
    ...
    <div class="flex h-14 items-center justify-between border-b px-4 text-sm">
      ...
      <div class="space-x-2">
        <Button variant="outline" size="sm" class="text-xs" :disabled="exportIsLoading" @click="onExportData">
    ...
    <div class="flex h-14 items-center justify-between border-b px-1 text-sm">
      <div class="no-scrollbar flex items-center overflow-y-scroll">
        <div v-for="(projectWorkflow, index) in projectWorkflows" :key="index" class="py-2">
          <NuxtLinkLocale
            :to="`/projects/${projectId}/workflows/${projectWorkflow.id}`"
            class="mx-2 flex max-w-72 rounded-lg border px-3 py-2 text-xs"
            :class="{ '-mt-1 bg-white shadow-md': projectWorkflow.id === workflowId }"
          >
