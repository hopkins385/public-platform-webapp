...
export class CreatesNewUserAction {
  ...
  async createDefaultProject({ teamId, userId }: NewProjectDto) {
    ...
    return { teamId, userId, projectId: project.id };
  }
  ...
  private createPipeline() {
    ...
    const pipeLine = Pipe.create(maxRetries, async (p) => {
      p.addStep(async ({ userId, orgName }: PipelinePayload) => {
        if (!userId || !orgName) throw new Error('Missing required fields: userId or orgName');
        return this.updateUserName({ userId, orgName, firstName: 'User', lastName: 'Name' });
      });
      ...
      p.addStep(async ({ userId, organisationId }: PipelineStepResult) => {
        if (!userId || !organisationId) throw new Error('Missing required fields: userId or organisationId');
        return this.createTeam({ userId, name: 'Default Team', organisationId });
      });
      ...
      p.addStep(async ({ userId, teamId }: PipelineStepResult) => {
        if (!userId || !teamId) throw new Error('Missing required fields: userId or teamId');
        return this.createDefaultProject({ userId, teamId });
      });
      p.addStep(async ({ userId, teamId }: PipelineStepResult) => {
        if (!userId || !teamId) throw new Error('Missing required fields: userId or teamId');
