...
export const FluxProInputsSchema = z.object({
  ...
  width: z
    ...
    .default(1024)
    .optional()
    .describe('Width of the generated image in pixels. Must be a multiple of 32.'),
  height: z
    .number()
    .int()
    .min(256)
  ...
  seed: z.number().int().nullable().optional().describe('Optional seed for reproducibility.'),
  guidance: z
    .number()
    .min(1.5)
    .max(5.0)
  ...
  interval: z.number().min(1.0).max(4.0).default(2.0).optional().describe('Interval parameter for guidance control.'),
});
