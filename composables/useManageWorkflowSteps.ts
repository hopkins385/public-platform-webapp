...
export default function useManageWorkflowSteps() {
  ...
  async function updateInputSteps(workflowStepId: string, inputStepIds: string[]) {
    return $client.workflowStep.updateInputSteps.mutate(
      { workflowStepId, inputStepIds },
      {
        signal: ac.signal,
      },
    );
